...
/** The RichText field in a row of a List: the same methods as {@link RichText}, found inside the row. */
public class ListRichText
		<
				W extends ListWidget<W, RR, R>,
				RR extends PlatformRows<RR, W, R>,
				R extends PlatformRow<R, RR, W>
				>
		extends RichText<W, ListRichText<W, RR, R>> implements ParentRow<R> {

	private final R row;

	public ListRichText(W widget, String title, R row) {
		super(widget, title);
		this.row = row;
	}
