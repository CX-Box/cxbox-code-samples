...
public class OidcProvider {
	...
	/**
	 * The refresh token the application keeps in the browser, null when nobody is signed in
	 */
	@SneakyThrows
	public static String storedRefreshToken() {
		var storedUser = storedUser();
		return storedUser == null ? null : objectMapper.readTree(storedUser).path("refresh_token").asText(null);
	}

	/**
	 * Uses the refresh token once, from outside the browser, and returns the answer of the provider. With refresh token rotation
	 * the token is spent by this: the browser still holds it and does not know, so its own renewal will be a second use of the same
	 * token. That is how a test provokes the reuse without touching the application.
	 */
	@SneakyThrows
	public boolean spendRefreshToken(@NonNull String refreshToken) {
		var form = new okhttp3.FormBody.Builder()
				.add("grant_type", "refresh_token")
				.add("client_id", clientId)
				.add("refresh_token", refreshToken)
				.build();
		var request = new Request.Builder().url(tokenEndpoint).post(form).build();
		try (var response = new OkHttpClient().newCall(request).execute()) {
			log.info("refresh token spent outside the browser: {} -> {}", tokenEndpoint, response.code());
