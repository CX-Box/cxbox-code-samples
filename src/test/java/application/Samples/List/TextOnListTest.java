...
public class TextOnListTest extends BaseTestForSamples {
	...
	void read() {
//        var list = PlatformApp.screen("Text basic")
//              .secondLevelView("List")
//              .listInline("List title");
//        var row = list.rows().row(0);
//        row.text("Custom Field")
//              .checkValue(value -> assertThat(value).isEqualTo("Unlike a digital typeface, a metal font would not include a single definition of each character, but commonly used characters (such as vowels and periods) would have more physical type-pieces included."));
	}
	...
	void color() {
		var list = PlatformApp.screen("Text color")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().row(0);
		row.text("Custom Field")
	...
	void readonly() {
		var list = PlatformApp.screen("Text readonly")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().clickRow(0);
		row.text("Custom Field")
	...
	void filtration() {
		var list = PlatformApp.screen("Text filtration")
				.secondLevelView("List")
				.listInline("List title");
		list.headers().filter(fb -> fb.text("Custom Field", "dozen"));
		var values = list.rows().streamCurrentPage()
				.map(r -> r.text("Custom Field").getValue())
				.collect(Collectors.toList());
		assertThat(values).isEqualTo(List.of("A typeface may come in fonts of many weights, from ultra-light to extra-bold or black; four to six weights are not unusual, and a few typefa..."));
	}
	...
	void drillDown() {
		var list = PlatformApp.screen("Text drilldown")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().row(0);
		row.text("Custom Field")
	...
	void businessException() {
		var list = PlatformApp.screen("Text validation business exception")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().clickRow(0);
		row.text("Custom Field").setValue("1234");
		list.errorPopup()
	...
	void runtimeException() {
		var list = PlatformApp.screen("Text validation runtime exception")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().clickRow(0);
	...
	void confirm() {
		var list = PlatformApp.screen("Text validation confirm")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().clickRow(0);
		row.text("Custom Field").setValue("Test");
	...
	void fieldLevelValidationAnnotation() {
		var list = PlatformApp.screen("Text validation field level annotation")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().clickRow(0);
	...
	void fieldLevelValidation() {
		var list = PlatformApp.screen("Text validation field level dynamic")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().clickRow(0);
		var customField = row.text("Custom Field");
		...
		editRow.text("Custom FieldAdditional")
				.checkRequired(message -> assertThat(message).isEqualTo(Text.textOnlyLetters("customFieldAdditional")));
	}
	...
	void sorting() {
		var list = PlatformApp.screen("Text sorting")
				.secondLevelView("List")
				.listInline("List title");
		...
		var valuesAfter = list.rows().streamCurrentPage()
				.map(r -> r.text("Custom Field").getValue())
				.collect(Collectors.toList());
		assertThat(valuesAfter).isNotEqualTo(valuesBefore);
	}
	...
	void required() {
		var list = PlatformApp.screen("Text required")
				.secondLevelView("List")
				.listInline("List title");
		var row = list.rows().clickRow(0);
