...
import org.demo.documentation.fields.suggestion.basic.microservice.MyExample3080SuggestionDTO_;
...
public class WidgetStylesDao extends AbstractAnySourceBaseDAO<WidgetStylesOutServiceDTO> implements AnySourceBaseDAO<WidgetStylesOutServiceDTO> {
	...
	@Override
	public String getId(final WidgetStylesOutServiceDTO entity) {
		return entity.getId();
	}

	@Override
	public void setId(final String id, final WidgetStylesOutServiceDTO entity) {
		entity.setId(id);
	}
	...
	@Override
	public WidgetStylesOutServiceDTO getByIdIgnoringFirstLevelCache(final BusinessComponent bc) {
		return restTemplate.exchange(
				fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl() + "/{id}").build()
						...
						.toUriString(),
				GET, null, WidgetStylesOutServiceDTO.class
		).getBody();
	}
	...
	// --8<-- [start:getList]
	@Override
	public Page<WidgetStylesOutServiceDTO> getList(final BusinessComponent bc, final QueryParameters queryParameters) {
		...
		//Filter
		List<String> filterCustomField = getFilterFieldName(queryParameters, MyExample3080SuggestionDTO_.customFieldSuggestion.getName(), "contains");
		...
		ResponseEntity<RestResponsePage<WidgetStylesOutServiceDTO>> responseEntity = restTemplate.exchange(
				urlTemplate,
				HttpMethod.GET,
				null,
	...
	@Override
	// --8<-- [start:create]
	public WidgetStylesOutServiceDTO create(BusinessComponent bc, WidgetStylesOutServiceDTO entity) {
		entity.setId(null);
		return restTemplate.exchange(
				fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl()).build().normalize().encode().toUriString(),
				POST, new HttpEntity<>(entity), WidgetStylesOutServiceDTO.class
		).getBody();
	}
	// --8<-- [end:create]
	...
	@Override
	// --8<-- [start:update]
	public WidgetStylesOutServiceDTO update(BusinessComponent bc, WidgetStylesOutServiceDTO entity) {
		return restTemplate.exchange(
				fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl()).build().normalize().encode().toUriString(),
				PUT, new HttpEntity<>(entity), WidgetStylesOutServiceDTO.class
		).getBody();
	}
	// --8<-- [end:update]
