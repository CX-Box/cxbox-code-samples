...
public class MyEntity3424Dao extends AbstractAnySourceBaseDAO<MyEntity3424OutServiceDTO> implements AnySourceBaseDAO<MyEntity3424OutServiceDTO> {
    ...
    @Override
    public String getId(final MyEntity3424OutServiceDTO entity) {
        return entity.getId();
    }

    @Override
    public void setId(final String id, final MyEntity3424OutServiceDTO entity) {
        entity.setId(id);
    }
    ...
    @Override
    public MyEntity3424OutServiceDTO getByIdIgnoringFirstLevelCache(final BusinessComponent bc) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getDataServerUrl() + "/{id}").build()
                        .expand(bc.getIdAsLong()).normalize().encode()
                        .toUriString(),
                GET, null, MyEntity3424OutServiceDTO.class
        ).getBody();
    }
    ...
    // --8<-- [start:getList]
    @Override
    public Page<MyEntity3424OutServiceDTO> getList(final BusinessComponent bc, final QueryParameters queryParameters) {
        ...
        if (filter.isPresent()) {
            ...
            ResponseEntity<RestResponsePage<MyEntity3424OutServiceDTO>> responseEntity = restTemplate.exchange(
                    urlTemplate,
                    HttpMethod.GET,
                    null,
    ...
    @Override
    // --8<-- [start:create]
    public MyEntity3424OutServiceDTO create(BusinessComponent bc, MyEntity3424OutServiceDTO entity) {
        entity.setId(null);
        return restTemplate.exchange(
                fromUriString(integrationConfig.getDataServerUrl()).build().normalize().encode().toUriString(),
                POST, new HttpEntity<>(entity), MyEntity3424OutServiceDTO.class
        ).getBody();
    }
    // --8<-- [end:create]
    ...
    @Override
    // --8<-- [start:update]
    public MyEntity3424OutServiceDTO update(BusinessComponent bc, MyEntity3424OutServiceDTO entity) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getDataServerUrl()).build().normalize().encode().toUriString(),
                PUT, new HttpEntity<>(entity), MyEntity3424OutServiceDTO.class
        ).getBody();
    }
    // --8<-- [end:update]
