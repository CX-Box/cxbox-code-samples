...
public class MyEntity3077Dao extends AbstractAnySourceBaseDAO<MyEntity3077OutServiceDTO> implements AnySourceBaseDAO<MyEntity3077OutServiceDTO> {
	...
	@Override
	public String getId(final MyEntity3077OutServiceDTO entity) {
		return entity.getId();
	}

	@Override
	public void setId(final String id, final MyEntity3077OutServiceDTO entity) {
		entity.setId(id);
	}
	...
	@Override
	public MyEntity3077OutServiceDTO getByIdIgnoringFirstLevelCache(final BusinessComponent bc) {
		return restTemplate.exchange(
				fromUriString(integrationConfig.getDataServerUrl() + "/{id}").build()
						.expand(bc.getIdAsLong()).normalize().encode()
						.toUriString(),
				GET, null, MyEntity3077OutServiceDTO.class
		).getBody();
	}
	...
	// --8<-- [start:getList]
	@Override
	public Page<MyEntity3077OutServiceDTO> getList(final BusinessComponent bc, final QueryParameters queryParameters) {
		...
		ResponseEntity<RestResponsePage<MyEntity3077OutServiceDTO>> responseEntity = restTemplate.exchange(
				urlTemplate,
				HttpMethod.GET,
				null,
	...
	@Override
	// --8<-- [start:create]
	public MyEntity3077OutServiceDTO create(BusinessComponent bc, MyEntity3077OutServiceDTO entity) {
		entity.setId(null);
		return restTemplate.exchange(
				fromUriString(integrationConfig.getDataServerUrl()).build().normalize().encode().toUriString(),
				POST, new HttpEntity<>(entity), MyEntity3077OutServiceDTO.class
		).getBody();
	}
	// --8<-- [end:create]
	...
	@Override
	// --8<-- [start:update]
	public MyEntity3077OutServiceDTO update(BusinessComponent bc, MyEntity3077OutServiceDTO entity) {
		return restTemplate.exchange(
				fromUriString(integrationConfig.getDataServerUrl()).build().normalize().encode().toUriString(),
				PUT, new HttpEntity<>(entity), MyEntity3077OutServiceDTO.class
		).getBody();
	}
	// --8<-- [end:update]
