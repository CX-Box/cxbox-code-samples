...
public class TaskMeta extends FieldMetaBuilder<TaskDTO> {

	@Override
	public void buildRowDependentMeta(RowDependentFieldsMeta<TaskDTO> fields, InnerBcDescription bcDescription, Long id, Long parentId) {
		fields.setEnabled(TaskDTO_.name,
				TaskDTO_.status);
		fields.setRequired(TaskDTO_.name,
				TaskDTO_.status);
	...
	public void buildIndependentMeta(FieldsMeta<TaskDTO> fields, InnerBcDescription bcDescription, Long parentId) {
		...
		fields.setForceActive(TaskDTO_.name,
				TaskDTO_.id,
				TaskDTO_.status);
