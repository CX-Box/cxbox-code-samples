...
import org.demo.documentation.fields.suggestion.color.MyEntity3082OutServiceDTO;
import org.demo.documentation.fields.suggestion.color.MyExample3082SuggestionDTO_;
...
public class MyEntity3082Dao extends AbstractAnySourceBaseDAO<MyEntity3082OutServiceDTO> implements AnySourceBaseDAO<MyEntity3082OutServiceDTO> {
    ...
    @Override
    public String getId(final MyEntity3082OutServiceDTO entity) {
        return entity.getId();
    }

    @Override
    public void setId(final String id, final MyEntity3082OutServiceDTO entity) {
        entity.setId(id);
    }
    ...
    @Override
    public MyEntity3082OutServiceDTO getByIdIgnoringFirstLevelCache(final BusinessComponent bc) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl() + "/{id}").build()
                        ...
                        .toUriString(),
                GET, null, MyEntity3082OutServiceDTO.class
        ).getBody();
    }
    ...
    // --8<-- [start:getList]
    @Override
    public Page<MyEntity3082OutServiceDTO> getList(final BusinessComponent bc, final QueryParameters queryParameters) {
        ...
        //Filter
        List<String> filterCustomField = getFilterFieldName(queryParameters, MyExample3082SuggestionDTO_.customFieldSuggestion.getName(), "contains");
        Optional<String> filter = filterCustomField.isEmpty() ? Optional.empty() : Optional.of(filterCustomField.get(0));

        //Sorting
        List<String> sortCustomField = getSortFieldName(queryParameters, MyExample3082SuggestionDTO_.customFieldSuggestion.getName());
        ...
        ResponseEntity<RestResponsePage<MyEntity3082OutServiceDTO>> responseEntity = restTemplate.exchange(
                urlTemplate,
                HttpMethod.GET,
                null,
    ...
    @Override
    // --8<-- [start:create]
    public MyEntity3082OutServiceDTO create(BusinessComponent bc, MyEntity3082OutServiceDTO entity) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl()).build().normalize().encode().toUriString(),
                POST, new HttpEntity<>(entity), MyEntity3082OutServiceDTO.class
        ).getBody();
    }
    // --8<-- [end:create]
    ...
    @Override
    // --8<-- [start:update]
    public MyEntity3082OutServiceDTO update(BusinessComponent bc, MyEntity3082OutServiceDTO entity) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl()).build().normalize().encode().toUriString(),
                PUT, new HttpEntity<>(entity), MyEntity3082OutServiceDTO.class
        ).getBody();
    }
    // --8<-- [end:update]
