...
import org.demo.documentation.fields.suggestion.colorconst.MyEntity3083OutServiceDTO;
...
public class MyEntity3083Dao extends AbstractAnySourceBaseDAO<MyEntity3083OutServiceDTO> implements AnySourceBaseDAO<MyEntity3083OutServiceDTO> {
    ...
    @Override
    public String getId(final MyEntity3083OutServiceDTO entity) {
        return entity.getId();
    }

    @Override
    public void setId(final String id, final MyEntity3083OutServiceDTO entity) {
        entity.setId(id);
    }
    ...
    @Override
    public MyEntity3083OutServiceDTO getByIdIgnoringFirstLevelCache(final BusinessComponent bc) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl() + "/{id}").build()
                        ...
                        .toUriString(),
                GET, null, MyEntity3083OutServiceDTO.class
        ).getBody();
    }
    ...
    // --8<-- [start:getList]
    @Override
    public Page<MyEntity3083OutServiceDTO> getList(final BusinessComponent bc, final QueryParameters queryParameters) {
        ...
        ResponseEntity<RestResponsePage<MyEntity3083OutServiceDTO>> responseEntity = restTemplate.exchange(
                urlTemplate,
                HttpMethod.GET,
                null,
    ...
    @Override
    // --8<-- [start:create]
    public MyEntity3083OutServiceDTO create(BusinessComponent bc, MyEntity3083OutServiceDTO entity) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl()).build().normalize().encode().toUriString(),
                POST, new HttpEntity<>(entity), MyEntity3083OutServiceDTO.class
        ).getBody();
    }
    // --8<-- [end:create]
    ...
    @Override
    // --8<-- [start:update]
    public MyEntity3083OutServiceDTO update(BusinessComponent bc, MyEntity3083OutServiceDTO entity) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getSuggestionMicroservicesDataServerUrl()).build().normalize().encode().toUriString(),
                PUT, new HttpEntity<>(entity), MyEntity3083OutServiceDTO.class
        ).getBody();
    }
    // --8<-- [end:update]
