...
public class MyExample5035DTO extends DataResponseDTO {
	...
	public MyExample5035DTO(MyEntity5035 entity) {
		...
		this.customFieldMultivalueHoverDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultivalueHoverList().stream().map(MyEntity5035Muti::getCustomField
		...
		this.customFieldMultivalueDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultivalueList().stream().map(MyEntity5035Muti::getCustomField
		).map(e -> e.toString()).collect(Collectors.joining(",")), 12);
