...
@Setter
@NoArgsConstructor
public class MyExample3329DTO extends DataResponseDTO {
	...
	public MyExample3329DTO(MyEntity3329 entity) {
		this.id = entity.getId().toString();
		this.customField = entity.getCustomField();
		this.customFieldMulti = entity.getCustomFieldMultiList().stream().collect(MultivalueField.toMultivalueField(
				e -> String.valueOf(e.getId()),
				MyEntity3329Multi::getCustomField
		));
		this.customFieldMultiDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultiList().stream().map(MyEntity3329Multi::getCustomField
