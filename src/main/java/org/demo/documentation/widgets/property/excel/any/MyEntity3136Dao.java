...
public class MyEntity3136Dao extends AbstractAnySourceBaseDAO<MyEntity3136OutServiceDTO> implements AnySourceBaseDAO<MyEntity3136OutServiceDTO> {
    ...
    @Override
    public String getId(final MyEntity3136OutServiceDTO entity) {
        return entity.getId();
    }

    @Override
    public void setId(final String id, final MyEntity3136OutServiceDTO entity) {
        entity.setId(id);
    }
    ...
    @Override
    public MyEntity3136OutServiceDTO getByIdIgnoringFirstLevelCache(final BusinessComponent bc) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getDataServerUrl() + "/{id}").build()
                        .expand(bc.getIdAsLong()).normalize().encode()
                        .toUriString(),
                GET, null, MyEntity3136OutServiceDTO.class
        ).getBody();
    }
    ...
    // --8<-- [start:getList]
    @Override
    public Page<MyEntity3136OutServiceDTO> getList(final BusinessComponent bc, final QueryParameters queryParameters) {
        ...
        ResponseEntity<RestResponsePage<MyEntity3136OutServiceDTO>> responseEntity = restTemplate.exchange(
                urlTemplate,
                HttpMethod.GET,
                null,
    ...
    @Override
    // --8<-- [start:update]
    public MyEntity3136OutServiceDTO update(BusinessComponent bc, MyEntity3136OutServiceDTO entity) {
        return restTemplate.exchange(
                fromUriString(integrationConfig.getDataServerUrl()).build().normalize().encode().toUriString(),
                PUT, new HttpEntity<>(entity), MyEntity3136OutServiceDTO.class
        ).getBody();
    }
    // --8<-- [end:update]
