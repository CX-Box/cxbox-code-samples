...
@Setter
@NoArgsConstructor
public class MyExample3010DTO extends DataResponseDTO {
    ...
    public MyExample3010DTO(MyEntity3010 entity) {
        ...
        this.customFieldMultivalueHoverDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultivalueHoverList().stream().map(MyEntity3010MultiHover::getCustomField
        ...
        this.customFieldMultivalueDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultivalueList().stream().map(MyEntity3010Multi::getCustomField
        ).map(e -> e.toString()).collect(Collectors.joining(",")), 12);
