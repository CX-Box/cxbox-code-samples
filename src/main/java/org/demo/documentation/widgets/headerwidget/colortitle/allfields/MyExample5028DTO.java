...
@Setter
@NoArgsConstructor
public class MyExample5028DTO extends DataResponseDTO {
    ...
    public MyExample5028DTO(MyEntity5028 entity) {
        ...
        this.customFieldMultivalueHoverDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultivalueHoverList().stream().map(MyEntity5028Multi::getCustomField
        ...
        this.customFieldMultivalueDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultivalueList().stream().map(MyEntity5028Multi::getCustomField
        ).map(e -> e.toString()).collect(Collectors.joining(",")), 12);
