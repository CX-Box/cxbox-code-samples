...
@Setter
@NoArgsConstructor
public class MyExample3192DTO extends DataResponseDTO {
	...
	public MyExample3192DTO(MyEntity3192 entity) {
		...
		this.customFieldMultivalueHoverDisplayedKeyDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultivalueHoverDisplayedKeyList().stream().map(MyEntity3192MultiHover::getCustomField
		...
		this.customFieldMultivalueDisplayedKey = StringUtils.abbreviate(entity.getCustomFieldMultivalueList().stream().map(MyEntity3192Multi::getCustomField
		).map(e -> e.toString()).collect(Collectors.joining(",")), 12);
