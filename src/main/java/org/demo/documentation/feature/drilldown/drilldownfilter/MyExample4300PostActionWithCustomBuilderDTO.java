...
public class MyExample4300PostActionWithCustomBuilderDTO extends DataResponseDTO {
	...
	public MyExample4300PostActionWithCustomBuilderDTO(MyEntity4300PostActionWithCustomBuilder entity) {
		...
		this.customFieldInlinePickList = Optional.ofNullable(entity).map(MyEntity4300PostActionWithCustomBuilder::getMyEntity4300InlinePickList)
				.map(MyEntity4300InlinePickList::getValue).orElse(null);
		this.customFieldInlinePickListId = Optional.ofNullable(entity).map(MyEntity4300PostActionWithCustomBuilder::getMyEntity4300InlinePickList)
				.map(BaseEntity::getId).orElse(null);

		this.customFieldPickList = Optional.ofNullable(entity).map(MyEntity4300PostActionWithCustomBuilder::getMyEntity4300PickList)
				.map(MyEntity4300PickList::getValue).orElse(null);
		this.customFieldPickListId = Optional.ofNullable(entity).map(MyEntity4300PostActionWithCustomBuilder::getMyEntity4300PickList)
				.map(BaseEntity::getId).orElse(null);
