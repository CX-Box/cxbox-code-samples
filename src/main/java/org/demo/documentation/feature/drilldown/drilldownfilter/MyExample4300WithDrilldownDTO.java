...
public class MyExample4300WithDrilldownDTO extends DataResponseDTO {
	...
	public MyExample4300WithDrilldownDTO(MyEntityWithDrilldown4300 entity) {
		...
		this.customFieldInlinePickList = Optional.of(entity).map(MyEntityWithDrilldown4300::getMyEntity4300InlinePickList)
				.map(MyEntity4300InlinePickList::getValue).orElse(null);
		this.customFieldInlinePickListId = Optional.of(entity).map(MyEntityWithDrilldown4300::getMyEntity4300InlinePickList)
				.map(BaseEntity::getId).orElse(null);

		this.customFieldPickList = Optional.of(entity).map(MyEntityWithDrilldown4300::getMyEntity4300PickList).map(MyEntity4300PickList::getValue)
				.orElse(null);
		this.customFieldPickListId = Optional.of(entity).map(MyEntityWithDrilldown4300::getMyEntity4300PickList).map(BaseEntity::getId)
				.orElse(null);
