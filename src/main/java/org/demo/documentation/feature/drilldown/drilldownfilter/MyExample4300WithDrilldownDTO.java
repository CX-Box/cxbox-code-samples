...
@Setter
@NoArgsConstructor
public class MyExample4300WithDrilldownDTO extends DataResponseDTO {
	...
	@SearchParameter(name = "multivalueList.id", multiFieldKey = LongValueProvider.class, provider = MultiFieldValueProvider.class)
	private MultivalueField customFieldMultivalue;

	private String customFieldMultivalueDisplayKey;

	@SearchParameter(name = "multivalueHoverList.id", multiFieldKey = LongValueProvider.class, provider = MultiFieldValueProvider.class)
	...
	public MyExample4300WithDrilldownDTO(MyEntityWithDrilldown4300 entity) {
		...
		this.customFieldMultivalue = entity.getMultivalueList().stream().collect(MultivalueField.toMultivalueField(
				e -> String.valueOf(e.getId()), e -> e.getValue()
		));
		this.customFieldMultivalueDisplayKey = entity.getMultivalueList().stream().map(e -> e.getValue()).collect(Collectors.joining(","));
		this.customFieldMultivalueHover = entity.getMultivalueHoverList().stream().collect(MultivalueField.toMultivalueField(
				e -> String.valueOf(e.getId()), e -> e.getValue()
		));
		this.customFieldMultivalueHoverDisplayKey = entity.getMultivalueHoverList().stream().map(e -> e.getValue()).collect(Collectors.joining(","));

		this.customFieldInlinePickList = Optional.ofNullable(entity).map(MyEntityWithDrilldown4300::getMyEntity4300InlinePickList)
				.map(MyEntity4300InlinePickList::getValue).orElse(null);
		this.customFieldInlinePickListId = Optional.ofNullable(entity).map(MyEntityWithDrilldown4300::getMyEntity4300InlinePickList)
				.map(BaseEntity::getId).orElse(null);

		this.customFieldPickList = Optional.ofNullable(entity).map(MyEntityWithDrilldown4300::getMyEntity4300PickList).map(MyEntity4300PickList::getValue)
				.orElse(null);
		this.customFieldPickListId = Optional.ofNullable(entity).map(MyEntityWithDrilldown4300::getMyEntity4300PickList).map(BaseEntity::getId)
				.orElse(null);
