...
* APP_URL - the url where the tests will be run (default - https://code-samples.cxbox.org/ui/#/)
* CXBOX_RECORDER - true/false indicates whether video will be recorded for dropped tests.
* CXBOX_LOGGER - true/false added logs.
* KEYCLOAK_ADMIN_USER, KEYCLOAK_ADMIN_PASSWORD - admin account of the Keycloak the application uses (KC_BOOTSTRAP_ADMIN_USERNAME / KC_BOOTSTRAP_ADMIN_PASSWORD in docker-compose-test.yml). Enables application.Samples.Feature.signInAgain (skipped otherwise): every run creates its own users with a random suffix, works with their client roles and deletes them at the end; nothing shared (the client, the realm, the demo user) is changed, so runs can go in parallel against the same Keycloak. The tests of the token renewal across tabs and of the websocket (TokenRenewalAcrossTabsTest, WebSocketSessionTest) wait the real access token lifetime out; the compose Keycloak realm sets it to 2 minutes with refresh token rotation on, a shared Keycloak with a longer lifetime just makes them slower.
