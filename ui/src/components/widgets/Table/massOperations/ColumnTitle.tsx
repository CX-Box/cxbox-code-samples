...
const ColumnTitle: React.FC<ColumnTitleProps> = ({ widgetName, bcName, filterable, fieldName, title }) => {
    ...
    const filter = useAppSelector(state => state.screen.filters[bcName]?.find(item => item.fieldName === fieldName))
    const { visibility, changeVisibility } = useVisibility(false)
    const filterApplied = (filter?.value?.toString()?.length as number) > 0 || Array.isArray(filter?.value)
